...
interface CrystalReportElementsWindowProps {
  show: boolean;
  onClose: () => void;
...
export const CrystalReportElementsWindow: React.FC<CrystalReportElementsWindowProps> = ({
  show,
  onClose,
  windowState,
  setWindowState
  ...
  const elements = [
    'Unit Price', 'Line Total', 'BP Code', 'Employee', 'Tax Number', 
    'VAT ID', 'Delivery Date', 'PO No.', 'Document Number', 'Item Text', 
    'Discount', 'Total Net', 'Total Gross', 'Tax', 'Payment Terms', 
    'Delivery Terms', 'Quotation', 'Sales Order', 'Delivery Note', 
    'Invoice', 'Purchase Order', 'Thank you for your purchase.', 
    'Best regards', 'Reservation of Title', 'Phone', 'Managing Director', 
    'Account Number', 'Bank Number', 'Purchase Quotation', 'Proforma Invoice', 
    'Down Payment Invoice', 'Reserve Invoice', 'Delivery Address', 
    'Billing Address', 'Your Contact', 'Your Reference', 'Currency'
  ];
  ...
  return (
    ...
    >
      <div onMouseDown={handleDrag} className="h-[26px] bg-gradient-to-b from-[#fefefe] to-[#d1d1d1] flex items-center justify-between px-2 cursor-default shrink-0 border-b border-gray-400">
        <span className="text-black font-medium text-[11.5px] tracking-tight">Default Elements for SAP Crystal Reports - Setup</span>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar bg-[repeating-linear-gradient(white,white_19px,#f5f5f5_19px,#f5f5f5_20px)]">
            {elements.map((el, i) => (
              <div key={i} className="flex border-b border-gray-200 divide-x divide-gray-200 h-[20px] items-center hover:bg-blue-50">
                <div className="w-[30px] px-1 text-center text-gray-400 font-bold">{i + 4}</div>
                <div className="flex-1 px-2 text-[10.5px] text-gray-800">{el}</div>
              </div>
            ))}
          </div>
        </div>
