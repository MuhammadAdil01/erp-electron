...
export const SAPLinksWindow: React.FC<SAPLinksWindowProps> = ({
  show,
  onClose,
  windowState,
  setWindowState
  ...
  const sapActionButtonStyle = "px-4 py-0.5 bg-gradient-to-b from-[#fff6d5] via-[#ffec99] to-[#ffd700]/60 border border-gray-500 text-[11px] font-bold shadow-sm rounded-[1px] min-w-[70px] hover:brightness-95 active:shadow-inner h-[20px] flex items-center justify-center";

  const tableData = [
    { url: 'https://www.sap.com/documents/2017/06/484c875d-c37c-0010-82c7-eda71af511fa.html', desc: 'SAP Business One Road Map', provider: 'SAP' },
    { url: 'http://help.sap.com/businessone', desc: 'SAP Help Portal', provider: 'SAP' },
    { url: 'http://support.sap.com/businessone', desc: 'SAP Business One Support', provider: 'SAP' },
    { url: 'https://influence.sap.com/go/SAPBusinessOne', desc: 'SAP Business One Customer Influence', provider: 'SAP' },
    { url: 'https://training.sap.com/businessone', desc: 'SAP Business One Training', provider: 'SAP' },
    { url: 'https://blogs.sap.com/tag/tipoftheweek/', desc: 'SAP Business One Tips', provider: 'SAP' },
  ];
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5">
          <span className="text-black font-medium text-[11.5px] tracking-tight">SAP Links - Setup</span>
        </div>
      ...
      <div className="flex-1 overflow-hidden flex flex-col p-0.5 bg-white border border-gray-400 shadow-inner m-1.5">
          ...
          <table className="w-full text-[11px] border-collapse min-w-max">
            <thead className="sticky top-0 z-10 border-b border-gray-400 shadow-sm">
              <tr>
                ...
                <th className={`${headerStyle} w-1/2`}>Link Address</th>
                <th className={`${headerStyle} w-1/3`}>Description</th>
                <th className={`${headerStyle} w-32`}>Provider</th>
              </tr>
            </thead>
            <tbody>
              {tableData.map((row, i) => (
                <tr key={i} className={`h-[18px] border-b border-gray-200 hover:bg-[#ffed99]/30 cursor-default ${i % 2 === 0 ? 'bg-white' : 'bg-[#f8f8f8]'}`}>
                  ...
                  <td className={`${cellStyle} text-blue-700 underline cursor-pointer`}>{row.url}</td>
                  <td className={cellStyle}>{row.desc}</td>
                  <td className={cellStyle}>{row.provider}</td>
                </tr>
              ))}
              {/* Row 7 - Yellow Highlighted as per SAP style */}
              <tr className="h-[18px] border-b border-gray-200 bg-[#fffbd5]">
                <td className={`${cellStyle} text-center bg-gray-50 border-r-gray-400`}>7</td>
              ...
              {Array.from({ length: 15 }).map((_, i) => (
                <tr key={i + 7} className={`h-[18px] border-b border-gray-100 ${ (i + 7) % 2 === 0 ? 'bg-white' : 'bg-[#f8f8f8]'}`}>
      ...
      <div className="p-2 pt-0 flex bg-[#ececec]">
