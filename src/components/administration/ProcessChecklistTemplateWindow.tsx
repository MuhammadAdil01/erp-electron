...
interface ProcessChecklistTemplateWindowProps {
  show: boolean;
  onClose: () => void;
...
export const ProcessChecklistTemplateWindow: React.FC<ProcessChecklistTemplateWindowProps> = ({
  show,
  onClose,
  windowState,
  setWindowState
  ...
  const sapInputStyle = "w-full h-[18px] border border-gray-400 px-1 text-[11px] outline-none focus:border-orange-400 font-normal";
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5">
          <span className="text-black font-medium text-[11.5px] tracking-tight">Process Checklist Template</span>
        </div>
      ...
      <div className="flex-1 p-6 bg-[#f8f8f8] mx-1.5 my-1.5 border border-gray-300 shadow-sm overflow-hidden flex flex-col gap-3">
        <div className="grid grid-cols-[100px_120px_1fr] gap-x-4 gap-y-1.5 items-center">
           <span className={sapLabelStyle}>Code</span>
           <input type="text" className={sapYellowInputStyle} />
           <button className={`${sapButtonStyle} ml-auto w-full`}>Load from XML</button>

           <span className={sapLabelStyle}>Description</span>
           <input type="text" className={`${sapInputStyle} col-span-2`} />
        </div>

        <div className="flex gap-1.5 mt-auto">
           <button onClick={onClose} className={sapActionButtonStyle}>Add</button>
