...
const YellowArrow = () => (
  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="shrink-0">
    <path d="M12 4L10.59 5.41L16.17 11H4V13H16.17L10.59 18.59L12 20L20 12L12 4Z" fill="#FFA000"/>
  </svg>
);

export const DocumentJournalCriteriaWindow: React.FC<Props> = ({ windowState, onClose, onUpdateState, onFocus }) => {
  const accounts = [
    { id: '1', name: 'Assets' },
  ...
  return (
    <ResizableCriteriaWindow
      title="Document Journal - Selection Criteria"
    ...
    >
      <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar flex flex-col p-2 gap-2 text-[10.5px]">
         {/* Top Section */}
            ...
            <div className="flex-[1.2] flex flex-col pt-1">
               <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1.5">
                     <input type="checkbox" defaultChecked className="w-3 h-3" />
                     <span className="text-gray-800">Accounts</span>
                  </div>
                  <div className="flex items-center gap-1.5">
               ...
               {/* Table */}
               <div className="h-[100px] border border-gray-400 bg-white overflow-hidden flex flex-col relative w-full font-sans">
                  {/* Header */}
         ...
         <div className="flex items-start mt-2">
            <span className="w-[60px] text-gray-800 shrink-0">Selection</span>
            <div className="flex flex-col gap-1 flex-1">
               {[
                  { id: 'posting', label: 'Posting Date', checked: true, from: '01.07.25', to: '30.06.26' },
                  { id: 'creation', label: 'Creation Date', checked: false, from: '01.07.25', to: '30.06.26' },
                  { id: 'document', label: 'Document Date', checked: false, from: '01.07.25', to: '30.06.26' }
               ].map((row, i) => (
                  <div key={row.id} className="flex items-center gap-2">
                     ...
                     <div className="relative w-[75px]">
                        <input type="text" defaultValue={row.from} className="w-full h-[18px] border border-gray-400 bg-white px-1 outline-none text-right pr-1" />
                     </div>
                     <span className="text-gray-800 min-w-[15px] ml-1">To</span>
                     <div className="relative w-[75px]">
                        <input type="text" defaultValue={row.to} className="w-full h-[18px] border border-gray-400 bg-white px-1 outline-none text-right pr-1" />
                     </div>
                     {i === 2 && (
                        <button className="ml-auto w-[70px] h-[20px] bg-gradient-to-b from-[#ffea99] to-[#ffd000] border border-gray-500 rounded-[2px] shadow-sm hover:from-[#ffebaa] active:shadow-inner">
                           Expanded
                        </button>
                     )}
                  </div>
         ...
         {/* Output Mode & Checkboxes Section */}
         <div className="flex gap-4 mt-2">
            
            {/* Left Column */}
            <div className="flex-[1] flex flex-col gap-2">
               <div className="flex items-center gap-3">
                  <span className="text-gray-800">Output Mode</span>
                  <div className="relative w-[120px]">
                     <input type="text" defaultValue="Detailed" className="w-full h-[18px] border border-gray-400 bg-white px-1 outline-none cursor-default" readOnly />
                     <div className="absolute right-0 top-0 bottom-0 w-[14px] flex items-center justify-center bg-gray-200 border-l border-gray-400 cursor-pointer">
                        <ChevronDown className="w-2.5 h-2.5 text-gray-600" />
                     </div>
                  </div>
               ...
               <div className="flex flex-col gap-1 mt-1">
                  <div className="flex items-center gap-1.5">
                     <input type="radio" name="total" defaultChecked className="w-3 h-3" />
                     <span className="text-gray-800">No Total</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                     <input type="radio" name="total" className="w-3 h-3" />
                     <span className="text-gray-800">Month (Posting Date)</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                     <input type="radio" name="total" className="w-3 h-3" />
                     <span className="text-gray-800">Month (Posting Date) and Original Journal</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                     <input type="radio" name="total" className="w-3 h-3" />
                     <span className="text-gray-800">Period (Posting Date) and Original Journal</span>
                  </div>
               </div>
            </div>
            ...
            <div className="flex-[1.2] flex flex-col gap-1">
               ...
               <div className="flex items-start gap-1.5">
                  <input type="checkbox" className="w-3 h-3 mt-[1px]" />
                  <span className="text-gray-800 leading-snug">Display OB for Periods</span>
               </div>

               <div className="mt-4 flex flex-col gap-1">
                  <div className="flex items-start gap-1.5">
                     <input type="checkbox" className="w-3 h-3 mt-[1px]" />
                     <span className="text-gray-800 leading-snug">Ignore Adjustments</span>
                  </div>
                  <div className="flex items-start gap-1.5">
                     <input type="checkbox" className="w-3 h-3 mt-[1px]" />
                     <span className="text-gray-800 leading-snug">Display Installments in One Row</span>
                  </div>
               </div>

               <div className="mt-2 relative w-full">
                  <div className="relative w-full">
                     <input type="text" defaultValue="Display All Transactions" className="w-full h-[18px] border border-gray-400 bg-white px-1 outline-none cursor-default" readOnly />
                     <div className="absolute right-0 top-0 bottom-0 w-[14px] flex items-center justify-center bg-gray-200 border-l border-gray-400 cursor-pointer">
                        <ChevronDown className="w-2.5 h-2.5 text-gray-600" />
                     </div>
                  </div>
         ...
         {/* Numbering and Sort Section */}
         <div className="flex flex-col gap-1 mt-1">
            <div className="flex items-center gap-4">
               <span className="text-gray-800 font-medium">Numbering for Detailed Mode</span>
               <span className="text-gray-800 ml-4">First Sequential No.</span>
               <input type="text" defaultValue="1" className="w-[80px] h-[18px] border border-gray-400 bg-white px-1 outline-none text-right" />
               <span className="text-gray-800 ml-4">No. on First Printed Page</span>
               <input type="text" defaultValue="1" className="w-[80px] h-[18px] border border-gray-400 bg-white px-1 outline-none text-right" />
            </div>

            <div className="flex items-start gap-1.5 mt-1">
               <input type="checkbox" className="w-3 h-3 mt-[1px]" />
               <span className="text-gray-800 leading-snug">Sort</span>
            </div>
         </div>
      </div>
    </ResizableCriteriaWindow>
