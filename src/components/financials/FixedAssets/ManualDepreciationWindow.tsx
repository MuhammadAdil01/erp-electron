...
export const ManualDepreciationWindow: React.FC<Props> = (props) => {
  const [tab, setTab] = useState('Contents');
  const [useProjects, setUseProjects] = useState(true);
  const [useDistribution, setUseDistribution] = useState(true);

  return (
    <FixedAssetWindowShell title="Manual Depreciation" {...props} minWidth={850}>
      {/* Header */}
      <div className="grid grid-cols-2 gap-x-10 px-4 py-3 bg-[#f0f0f0] border-b border-gray-300 shrink-0">
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownInput value="Ordinary Depreciation" className="flex-1"/>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="flex-1 border border-gray-400 mx-4 mb-2 bg-white shadow-sm overflow-auto custom-scrollbar" style={{minHeight:0}}>
          <table className="w-full border-collapse" style={{minWidth:750}}>
            <thead className="sticky top-0 bg-[#e5e5e5] z-10 border-b border-gray-400">
              <tr>
                {['#','Asset No.','Asset Description','Total (LC)','Distr. Rule','Project','Remarks'].map((h,i)=>(
            ...
            <tbody>
              <tr className="border-b border-gray-200 h-6">
                ...
                {Array.from({length:5}).map((_,i)=>(
                  <td key={i} className="border-r border-gray-200 px-1"><div className="h-4 border border-gray-200 bg-white"/></td>
                ))}
              </tr>
              {Array.from({length:12}).map((_,i)=>(
                <tr key={i} className="border-b border-gray-100 h-6">
                  {Array.from({length:7}).map((_,j)=><td key={j} className="border-r border-gray-100"/>)}
                </tr>
              ))}
            </tbody>
          </table>
