...
interface FinalSettlementWindowProps {
  show: boolean;
  onClose: () => void;
...
export const FinalSettlementWindow: React.FC<FinalSettlementWindowProps> = ({ 
  show, 
  onClose, 
  windowState, 
  setWindowState 
  ...
  const handleResize = (direction: string) => (e: React.MouseEvent) => {
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
      ...
      setWindowState(prev => {
        ...
        if (direction.includes('s')) newHeight = Math.max(480, startHeight + deltaY);
        ...
        if (direction.includes('n')) {
          ...
          if (newHeight >= 480) newY = startYPos + deltaY;
          else newHeight = 480;
        }
  ...
  const headers = ["#", "LogInst", "Joining Date", "Gross Salary", "Years of Service", "Gratuity", "E..."];
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className="text-white font-semibold text-[11.5px] tracking-wide drop-shadow-[0_1px_1px_rgba(0,0,0,0.8)] truncate">Final Settlement</span>
        </div>
        <WindowControls 
      ...
      <div className="flex-1 flex flex-col overflow-auto custom-scrollbar bg-[#f0f0f0]">
        ...
        <div className="grid grid-cols-2 gap-x-12 p-4 pt-4">
           ...
           <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center">
                 <label className="text-gray-700 w-[100px]">Employee Code</label>
                 <input type="text" className="flex-1 bg-white border border-gray-400 h-[18px] px-1 shadow-[inset_1px_1px_1px_rgba(0,0,0,0.05)] outline-none" />
              </div>
              <div className="flex items-center">
              ...
              <div className="flex items-center">
                 <label className="text-gray-700 w-[100px]">Stage</label>
                 <input type="text" className="flex-1 bg-white border border-gray-400 h-[18px] px-1 shadow-[inset_1px_1px_1px_rgba(0,0,0,0.05)] outline-none" />
              </div>
           </div>
        </div>
              ...
              <table className="w-full border-collapse table-fixed">
                 ...
                 <tbody>
                    <tr className="border-b border-gray-100 h-[20px] bg-blue-50/10">
                      <td className="bg-[#f5f5f5] border-r border-gray-300 text-center text-gray-500 font-bold text-[10px]">1</td>
                      <td className="border-r border-gray-100"></td>
                      <td className="border-r border-gray-100 px-1 py-0.5"></td>
                      <td className="border-r border-gray-100 px-1 py-0.5 text-right pr-6 text-gray-800 text-[9.5px]">0.00</td>
                      <td className="border-r border-gray-100 px-1 py-0.5 text-right pr-6 text-gray-800 text-[9.5px]">0.00</td>
                      <td className="border-r border-gray-100 px-1 py-0.5 text-right pr-6 text-gray-800 text-[9.5px]">0.00</td>
